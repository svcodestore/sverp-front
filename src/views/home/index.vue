<!--
 * @Author: yanbuw1911
 * @Date: 2020-12-09 19:48:29
 * @LastEditTime: 2021-02-05 08:23:01
 * @LastEditors: yanbuw1911
 * @Description:
 * @FilePath: /sverp-front/src/views/home/index.vue
-->
<template>
  <div>
    <a-row :gutter="20">
      <a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="12">
        <a-card :loading="cardLoading">
          <template #title>
            <div style="display: flex;">
              <div>
                <span>
                  {{ $t('home.quickNav') }}
                </span>
                <svg
                  t="1609309374556"
                  class="icon"
                  viewBox="0 0 1024 1024"
                  version="1.1"
                  xmlns="http://www.w3.org/2000/svg"
                  p-id="3464"
                  width="16"
                  height="16"
                >
                  <path
                    d="M86.96201 729.043695a26.022285 26.022285 0 0 1 38.613713 0 27.701142 27.701142 0 0 1 0 38.613713L58.001725 835.231406a26.861714 26.861714 0 0 1-39.033427 0 27.701142 27.701142 0 0 1 0-38.613713z"
                    fill="#4E8CEE"
                    p-id="3465"
                  ></path>
                  <path
                    d="M171.744294 597.673127c-3.357714 26.022285-3.357714 52.883999-5.876 78.906284l180.057424 180.477138c26.022285 0 52.464284-2.938 78.906283-6.295714a633.768555 633.768555 0 0 0 95.694855-19.306857 41.971427 41.971427 0 0 1 14.689999 0 46.16857 46.16857 0 0 1 44.489713 47.847427 306.39142 306.39142 0 0 1-23.084285 105.348283c-5.456286 11.752-10.912571 23.923714-17.627999 35.255999a182.155995 182.155995 0 0 0 76.807712 0 214.473994 214.473994 0 0 0 115.421426-57.500855v-2.098572a206.079709 206.079709 0 0 0 57.500855-106.187711 209.857137 209.857137 0 0 0-9.233714-121.297426 46.588284 46.588284 0 0 1 12.591428-49.945998l14.69-13.850571 11.752-11.752a679.097696 679.097696 0 0 0 177.119424-307.230849A685.39341 685.39341 0 0 0 1007.815129 16.788571a692.528553 692.528553 0 0 0-330.315135 6.715428 684.134268 684.134268 0 0 0-309.749134 178.378567l-13.011143 12.171714-13.011142 13.850571a45.329142 45.329142 0 0 1-51.624856 13.850571A209.857137 209.857137 0 0 0 168.386579 232.941422a204.400852 204.400852 0 0 0-105.348283 57.500856A216.99228 216.99228 0 0 0 4.698012 483.930559a280.78885 280.78885 0 0 1 37.774285-17.628 331.574277 331.574277 0 0 1 104.08914-23.503999 38.613713 38.613713 0 0 1 15.529428 0A45.329142 45.329142 0 0 1 192.310293 503.65713a713.514267 713.514267 0 0 0-20.565999 96.114568z"
                    fill="#1296db"
                    p-id="3466"
                  ></path>
                  <path
                    d="M255.687149 897.349119a27.281428 27.281428 0 0 1 38.613713 38.613713l-68.833141 67.993713a26.441999 26.441999 0 0 1-37.774285 0 26.022285 26.022285 0 0 1 0-38.613713zM171.324579 812.566836a27.281428 27.281428 0 1 1 38.613714 39.033427L55.48344 1007.314259a27.281428 27.281428 0 0 1-38.193999 0 26.861714 26.861714 0 0 1 0-41.971427z"
                    fill="#4E8CEE"
                    p-id="3467"
                  ></path>
                  <path
                    d="M878.962846 313.946277a170.403995 170.403995 0 1 1-169.984281-169.984281 167.88571 167.88571 0 0 1 169.984281 169.984281z"
                    fill="#FFFFFF"
                    p-id="3468"
                  ></path>
                  <path
                    d="M708.978565 198.524852a115.001711 115.001711 0 1 0 115.421426 115.421425 115.001711 115.001711 0 0 0-115.421426-115.421425z"
                    fill="#4E8CEE"
                    p-id="3469"
                  ></path>
                </svg>
              </div>
              <div style="margin-left: auto; font-size: 12px;" title="点击界面右上角的小星星来添加快捷入口吧！">
                <strong>点击</strong>
                <svg
                  t="1609228892668"
                  class="icon"
                  viewBox="0 0 1024 1024"
                  version="1.1"
                  xmlns="http://www.w3.org/2000/svg"
                  p-id="26414"
                  width="16"
                  height="16"
                >
                  <path
                    d="M129.832 884.872a381.336 81.488 0 1 0 762.672 0 381.336 81.488 0 1 0-762.672 0Z"
                    fill="#B8CBCD"
                    p-id="26415"
                  ></path>
                  <path
                    d="M208.52 243.224c-22.608-66.672 13.44-102.72 80.112-80.112l38.12 12.92L459.2 220.944c6.176 2.096 15.192 0.856 20.024-2.744l111.376-83.112 33.68-25.136c56.424-42.104 101.848-18.96 100.952 51.432l-0.512 40.024-1.784 139.96c-0.088 6.584 2.824 14.072 6.456 16.648 3.64 2.568 52.992 38.84 109.672 80.592l41.896 30.864c56.68 41.76 47.664 91.728-20.032 111.056l-41.832 11.936c-67.696 19.32-127.032 36.36-131.848 37.856-4.832 1.504-10.48 8.2-12.56 14.896-2.08 6.696-20.872 67.176-41.776 134.4l-11.424 36.736c-20.896 67.224-71.248 75.2-111.896 17.72l-28.4-40.144-78.176-110.544c-2.344-3.32-9.384-5.976-15.632-5.896l-139.36 1.784-41.216 0.528c-70.4 0.896-93.536-44.528-51.44-100.944l26.032-34.888L264.064 433.2c3.368-4.504 4.312-13.512 2.112-20.016l-45.12-133.04-12.536-36.92z"
                    fill="#F6E89A"
                    p-id="26416"
                  ></path>
                  <path
                    d="M566.6 896.488c-27.272 0-54.488-17.24-76.616-48.544l-28.4-40.144-75.072-106.16-181.768 2.176c-57.552 0-79.488-26.6-87.44-42.456-8.024-16.008-16.256-49.872 18.808-96.856l107.224-143.688a1.632 1.632 0 0 0-0.032-0.312l-57.528-169.584c-15.696-46.272-5.656-74.584 5.528-90.184 19.992-27.872 59.288-35.856 105.024-20.36l169.752 57.56 0.168-0.016 143.664-107.2c22.928-17.112 45.304-25.792 66.488-25.792 18.512 0 34.968 6.768 47.608 19.576 16.912 17.128 25.624 43.816 25.2 77.192l-0.832 64.472c-0.752 58.376-1.368 107.024-1.464 114.936 13.904 10.144 62.248 45.696 106.352 78.184l41.896 30.864c47.464 34.968 49.192 69.84 46.008 87.52-3.184 17.68-17 49.752-73.688 65.936l-41.832 11.936a35027.144 35027.144 0 0 0-128.04 36.728 0.856 0.856 0 0 0-0.072 0.152l-53.168 171.064c-14.392 46.384-42.744 72.992-77.768 73zM387.088 653.504c11.472 0 27.16 4.208 35.52 16.056l78.168 110.528 28.4 40.144c12.512 17.696 26.504 28.264 37.432 28.256 12.184 0 24.432-15.04 31.952-39.248l53.2-171.128c3.768-12.08 13.984-26.24 28.368-30.704 5.528-1.72 71.656-20.688 132.384-38.024l41.832-11.936c22.792-6.504 37.6-17.08 39.624-28.296 2.024-11.216-8.168-26.304-27.24-40.352l-41.896-30.864c-61.624-45.4-106.256-78.176-109.28-80.312-11.704-8.296-16.768-24.336-16.608-36.568l1.472-115.496 0.832-64.472c0.248-19.664-3.888-35.288-11.36-42.856-3.56-3.616-7.84-5.296-13.456-5.296-10.312 0-23.736 5.776-37.792 16.264L493.552 237.44c-10.912 8.136-28.664 10.784-42.08 6.232L280.92 185.84c-24.128-8.184-43.456-7.104-50.592 2.864-6.36 8.872-6.016 26.368 0.912 46.808l57.648 169.984c4.744 14 2.432 31.304-5.616 42.08L174.584 593.216c-14.136 18.944-19.504 36.376-14.368 46.632 5.016 10 21.664 15.976 44.528 15.976l42.952-0.536 139.368-1.784h0.024z"
                    fill="#8D4520"
                    p-id="26417"
                  ></path>
                  <path
                    d="M407.296 713.44a23.976 23.976 0 0 1-19.928-10.592l-20.152-29.944H149.624c-10.704 0-20.208-5.672-24.832-14.8-4.864-9.632-3.544-21.144 3.536-30.776l73.568-100.024c34.4-46.768 65.376-88.84 72.896-99.016a1.816 1.816 0 0 0-0.032-0.328c-1.736-5.064-21.912-58.88-49.016-130.296l-4.776-12.592c-22.096-58.224-4.672-87.296 7.688-99.448 11.44-11.248 38.048-27.168 87.744-7.344 57.136 22.792 103.344 40.96 116.336 45.872 1.712-0.664 4.232-1.984 6.384-3.872l15.472-13.584a24 24 0 1 1 31.656 36.08l-15.456 13.56c-12.872 11.304-33.4 20.168-50.24 14.456-7.664-2.6-68.536-26.64-121.928-47.936-19.36-7.728-32.184-7.072-36.312-3.016-5.168 5.088-6.272 22.336 3.536 48.192l4.776 12.592c22.424 59.08 47.152 124.728 49.72 132.304 4.744 14 2.432 31.312-5.624 42.08-3.328 4.472-36.712 49.8-74.152 100.704l-50.872 69.168h181.808c12.728 0 25.84 6.784 32.632 16.88l23.048 34.24a24 24 0 0 1-6.504 33.32 23.744 23.744 0 0 1-13.384 4.12z"
                    fill="#FFFFFF"
                    p-id="26418"
                  ></path>
                  <path
                    d="M599.296 895.168l-46.592-11.544 33.2-133.96c15.208-61.36 32.44-129.664 34.952-137.752 3.768-12.08 13.984-26.24 28.368-30.704 7.048-2.192 84.76-20.24 135.272-31.832l29.376-6.744c28.376-6.512 37.296-16.584 38.056-20.992 0.76-4.408-4.312-16.872-28.912-32.448l-23.728-15.032c-41.984-26.584-110.544-70.144-115.776-73.84-11.688-8.272-16.752-24.312-16.592-36.552 0.088-7.08 1.384-73.52 2.616-135.488l2.472-123.352 47.984 0.96-2.472 123.344c-1.384 69.672-2.456 125.056-2.592 134.32 12.888 8.352 56.024 35.848 110.048 70.056l23.728 15.032c50.568 32.024 53.368 64.552 50.536 81.096-2.824 16.544-16.288 46.296-74.632 59.68l-29.376 6.744c-64.416 14.784-114.64 26.584-128.552 30.056l-0.008 0.008c-1.712 5.552-16.184 62.416-34.168 134.984l-33.208 133.96z"
                    fill="#8D4520"
                    p-id="26419"
                  ></path>
                  <path
                    d="M566.6 896.488c-27.272 0-54.488-17.24-76.616-48.544l-28.4-40.144-75.072-106.16-181.768 2.176c-57.552 0-79.488-26.6-87.44-42.456-8.024-16.008-16.256-49.872 18.808-96.856l107.224-143.688a1.632 1.632 0 0 0-0.032-0.312l-57.528-169.584c-15.696-46.272-5.656-74.584 5.528-90.184 19.992-27.872 59.288-35.856 105.024-20.36l169.752 57.56 0.168-0.016 143.664-107.2c22.928-17.112 45.304-25.792 66.488-25.792 18.512 0 34.968 6.768 47.608 19.576 16.912 17.128 25.624 43.816 25.2 77.192l-0.832 64.472c-0.752 58.376-1.368 107.024-1.464 114.936 13.904 10.144 62.248 45.696 106.352 78.184l41.896 30.864c47.464 34.968 49.192 69.84 46.008 87.52-3.184 17.68-17 49.752-73.688 65.936l-41.832 11.936a35027.144 35027.144 0 0 0-128.04 36.728 0.856 0.856 0 0 0-0.072 0.152l-53.168 171.064c-14.392 46.384-42.744 72.992-77.768 73zM387.088 653.504c11.472 0 27.16 4.208 35.52 16.056l78.168 110.528 28.4 40.144c12.512 17.696 26.504 28.264 37.432 28.256 12.184 0 24.432-15.04 31.952-39.248l53.2-171.128c3.768-12.08 13.984-26.24 28.368-30.704 5.528-1.72 71.656-20.688 132.384-38.024l41.832-11.936c22.792-6.504 37.6-17.08 39.624-28.296 2.024-11.216-8.168-26.304-27.24-40.352l-41.896-30.864c-61.624-45.4-106.256-78.176-109.28-80.312-11.704-8.296-16.768-24.336-16.608-36.568l1.472-115.496 0.832-64.472c0.248-19.664-3.888-35.288-11.36-42.856-3.56-3.616-7.84-5.296-13.456-5.296-10.312 0-23.736 5.776-37.792 16.264L493.552 237.44c-10.912 8.136-28.664 10.784-42.08 6.232L280.92 185.84c-24.128-8.184-43.456-7.104-50.592 2.864-6.36 8.872-6.016 26.368 0.912 46.808l57.648 169.984c4.744 14 2.432 31.304-5.616 42.08L174.584 593.216c-14.136 18.944-19.504 36.376-14.368 46.632 5.016 10 21.664 15.976 44.528 15.976l42.952-0.536 139.368-1.784h0.024z"
                    fill="#8D4520"
                    p-id="26420"
                  ></path>
                </svg>
                <strong>添加</strong>
              </div>
            </div>
          </template>
          <a-card-grid
            class="fav-page-item"
            v-for="(item, idx) in favPages"
            :key="idx"
            @click="$router.push({ path: menuList.find(menu => menu.id == item.ifp_menu_id).mnu_path })"
            :title="$t(menuList.find(menu => menu.id == item.ifp_menu_id).mnu_code)"
          >
            {{ `${$t(menuList.find(menu => menu.id == item.ifp_menu_id).mnu_code)}` }}
          </a-card-grid>
        </a-card>
      </a-col>
      <a-col
        v-if="isShowSysRel === 'true'"
        :xs="24"
        :sm="24"
        :md="24"
        :lg="24"
        :xl="12">
        <a-card :loading="cardLoading" hoverable>
          <template #title>
            <span title="功能的更新修改和补丁等说明">系统更新信息</span>
            <a href="javascript:;" title="编写发布信息" v-if="userTitle === 'software developer'" @click="hdlWrite">
              <a-icon type="edit" v-if="!isWriting" />
              <a-icon type="save" v-else />
            </a>
          </template>
          <template #extra>
            <span>
              {{ modDate }}
            </span>
            <a href="javascript:;" @click="clsSysRelInfo">
              <a-icon type="close" />
            </a>
          </template>
          <div class="sysUpdateDesc" v-if="isWriting">
            <a-textarea v-model="modInfo" autoSize></a-textarea>
          </div>
          <div v-else v-html="modInfo"></div>
        </a-card>
      </a-col>
    </a-row>
  </div>
</template>

<script>
import { mapState } from 'vuex'

const indexedDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB
sessionStorage.setItem('isShowSysRel', true)

export default {
  data: () => ({
    isShowSysRel: 'true',
    cardLoading: true,
    isWriting: false,
    modDate: null,
    modInfo: ''
  }),
  computed: {
    ...mapState({
      favPages: state => state.user.favPages,
      menuList: state => state.permission.addRouters,
      userTitle: state => state.user.title
    })
  },
  async mounted () {
    await this.hdlIndexedDb()

    this.isShowSysRel = sessionStorage.getItem('isShowSysRel')

    this.cardLoading = false
  },
  methods: {
    clsSysRelInfo () {
      sessionStorage.setItem('isShowSysRel', false)
      this.isShowSysRel = 'false'
    },
    async hdlWrite () {
      if (this.isWriting) {
        await this.hdlIndexedDb(true)
      }

      this.isWriting = !this.isWriting
    },
    hdlIndexedDb (flag = false) {
      // 打开数据库，两个参数（数据库名字，版本号），如果数据库不存在则创建一个新的库
      const request = indexedDB.open('sysRelInfo', '1')
      // 数据库操作过程中出错，则错误回调被触发
      request.onerror = () => {
        console.log('连接 IndexedDb 出错')
      }
      // 数据库操作一切正常，所有操作后触发
      request.onsuccess = event => {
        const db = event.target.result
        // 数据读取
        const objectStore = db.transaction('release', 'readwrite').objectStore('release')

        // 更新数据
        if (flag === true) {
          const putReq = objectStore.put({
            id: 1,
            desc: this.modInfo,
            date: new Date().toLocaleString()
          })
          putReq.onsuccess = () => {
            if (putReq.result) {
              this.$message.success('保存成功')
            } else {
              this.$message.success('保存失败')
            }
          }
        }
        // 获取数据
        const getReq = objectStore.get(1)

        getReq.onsuccess = () => {
          const { desc, date } = getReq.result
          this.modDate = date
          this.modInfo = desc
        }
      }
      // 创建一个新的数据库或者修改数据库版本号时触发
      request.onupgradeneeded = () => {
        const db = request.result
        console.log(db)
        if (!db.objectStoreNames.contains('release')) {
          // 创建对象仓库用来存储数据，把id作为keyPath，keyPath必须保证不重复，相当于数据库的主键
          const objectStore = db.createObjectStore('release', { keyPath: 'id' })
          // 建立索引，name和age可能重复，因此unique设置为false
          objectStore.createIndex('desc', 'desc', { unique: false })
          objectStore.createIndex('date', 'date', { unique: false })
          // 确保在插入数据前对象仓库已经建立
          objectStore.transaction.oncomplete = () => {
            // 将数据保存到数据仓库
            var usersObjectStore = db.transaction('release', 'readwrite').objectStore('release')
            usersObjectStore.add({
              id: 1,
              desc: `<p>更新内容</p>
              <p>更新内容</p>
              <p>更新内容</p>`,
              date: new Date().toLocaleString()
            })
          }
        }
      }
    }
  }
}
</script>

<style lang="less" scoped>
/deep/.fav-page-item {
  width: 25%;
  text-align: center;
  letter-spacing: 5px;
  font-size: 18px;
  user-select: none;
  cursor: pointer;
  font-family: Tahoma, Helvetica, Arial, '\5b8b\4f53', sans-serif;
  word-break: keep-all;
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
}
</style>
