...
import SVPng from '../assets/starvincci'
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      // 动态主路由
      mainMenu: state => state.permission.addRouterTree,
  ...
  updated () {
    this.isFavoritePage = !!this.favPages.find(item => item.ifp_menu_id === String(this.$route.meta.id))
  },
  methods: {
    ...mapActions(['AddFavPage']),
    i18nRender,
    ...
    async handleFavoritePage () {
      ...
      this.AddFavPage(pages)
      this.isFavoritePage = !!this.favPages.find(item => item.ifp_menu_id === String(this.$route.meta.id))
    },
    async getFavPages () {
      await getUserFavoritePages(this.id || localStorage.getItem('id'))
        .then(res => res.result && this.AddFavPage(res.data))
        .catch(() => {})
      this.isFavoritePage = !!this.favPages.find(item => item.ifp_menu_id === String(this.$route.meta.id))
    }
  }
